...
#### 4. Excel 임포트 매핑 로직
헤더 고정(`일자(원본) / 사업장 / 활동 유형 / 설명 / 량 / 단위`)은 정해뒀는데,
...
- Excel 파일(`.xlsx`) 일괄 업로드 — 헤더 고정: `일자(원본) / 사업장 / 활동 유형 / 설명 / 량 / 단위`
...
### AI 사용에 대한 솔직한 회고                                 
- **잘 한 점**: AI가 짠 코드를 그대로 받지 않고 "왜 이렇게 짰는지", "내 의도와 맞는지" 매번 검증.
...
- [x] **Excel 임포트** (헤더는 고정 `일자(원본) / 사업장 / 활동 유형 / 설명 / 량 / 단위`, 공백/대소문자 무시 매칭)
- [ ] Docker Compose
- [ ] OpenAPI/Swagger
- [ ] 타 시스템과 비교
